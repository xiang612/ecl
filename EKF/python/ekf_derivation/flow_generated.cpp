// X axis
float HK0 = -Tbs(1,0)*q2 + Tbs(1,1)*q1 + Tbs(1,2)*q0;
float HK1 = Tbs(1,0)*q3 + Tbs(1,1)*q0 - Tbs(1,2)*q1;
float HK2 = Tbs(1,0)*q0 - Tbs(1,1)*q3 + Tbs(1,2)*q2;
float HK3 = HK0*vd + HK1*ve + HK2*vn;
float HK4 = 1.0F/range;
float HK5 = 2*HK4;
float HK6 = Tbs(1,0)*q1 + Tbs(1,1)*q2 + Tbs(1,2)*q3;
float HK7 = -HK0*ve + HK1*vd + HK6*vn;
float HK8 = HK0*vn - HK2*vd + HK6*ve;
float HK9 = -HK1*vn + HK2*ve + HK6*vd;
float HK10 = q0*q2;
float HK11 = q1*q3;
float HK12 = 2*Tbs(1,2);
float HK13 = q0*q3;
float HK14 = q1*q2;
float HK15 = 2*Tbs(1,1);
float HK16 = powf(q1, 2);
float HK17 = powf(q2, 2);
float HK18 = -HK17;
float HK19 = powf(q0, 2);
float HK20 = powf(q3, 2);
float HK21 = HK19 - HK20;
float HK22 = HK12*(HK10 + HK11) - HK15*(HK13 - HK14) + Tbs(1,0)*(HK16 + HK18 + HK21);
float HK23 = 2*Tbs(1,0);
float HK24 = q0*q1;
float HK25 = q2*q3;
float HK26 = -HK16;
float HK27 = -HK12*(HK24 - HK25) + HK23*(HK13 + HK14) + Tbs(1,1)*(HK17 + HK21 + HK26);
float HK28 = HK15*(HK24 + HK25) - HK23*(HK10 - HK11) + Tbs(1,2)*(HK18 + HK19 + HK20 + HK26);
float HK29 = 2*HK3;
float HK30 = 2*HK7;
float HK31 = 2*HK8;
float HK32 = 2*HK9;
float HK33 = HK22*P(0,4) + HK27*P(0,5) + HK28*P(0,6) + HK29*P(0,0) + HK30*P(0,1) + HK31*P(0,2) + HK32*P(0,3);
float HK34 = powf(range, -2);
float HK35 = HK22*P(4,6) + HK27*P(5,6) + HK28*P(6,6) + HK29*P(0,6) + HK30*P(1,6) + HK31*P(2,6) + HK32*P(3,6);
float HK36 = HK22*P(4,5) + HK27*P(5,5) + HK28*P(5,6) + HK29*P(0,5) + HK30*P(1,5) + HK31*P(2,5) + HK32*P(3,5);
float HK37 = HK22*P(4,4) + HK27*P(4,5) + HK28*P(4,6) + HK29*P(0,4) + HK30*P(1,4) + HK31*P(2,4) + HK32*P(3,4);
float HK38 = HK22*P(2,4) + HK27*P(2,5) + HK28*P(2,6) + HK29*P(0,2) + HK30*P(1,2) + HK31*P(2,2) + HK32*P(2,3);
float HK39 = HK22*P(3,4) + HK27*P(3,5) + HK28*P(3,6) + HK29*P(0,3) + HK30*P(1,3) + HK31*P(2,3) + HK32*P(3,3);
float HK40 = HK22*P(1,4) + HK27*P(1,5) + HK28*P(1,6) + HK29*P(0,1) + HK30*P(1,1) + HK31*P(1,2) + HK32*P(1,3);
float HK41 = HK4/(HK22*HK34*HK37 + HK27*HK34*HK36 + HK28*HK34*HK35 + HK29*HK33*HK34 + HK30*HK34*HK40 + HK31*HK34*HK38 + HK32*HK34*HK39 + R_LOS);
...
// Y axis
float HK0 = Tbs(0,1)*q1;
float HK1 = Tbs(0,2)*q0;
float HK2 = Tbs(0,0)*q2;
float HK3 = HK0 + HK1 - HK2;
float HK4 = Tbs(0,0)*q3;
float HK5 = Tbs(0,1)*q0;
float HK6 = Tbs(0,2)*q1;
float HK7 = HK4 + HK5 - HK6;
float HK8 = Tbs(0,0)*q0;
float HK9 = Tbs(0,2)*q2;
float HK10 = Tbs(0,1)*q3;
float HK11 = -HK10 + HK8 + HK9;
float HK12 = HK11*vn + HK3*vd + HK7*ve;
float HK13 = 1.0F/range;
float HK14 = 2*HK13;
float HK15 = Tbs(0,0)*q1 + Tbs(0,1)*q2 + Tbs(0,2)*q3;
float HK16 = HK15*vn + HK7*vd;
float HK17 = HK16 - HK3*ve;
float HK18 = HK15*ve + HK3*vn;
float HK19 = -HK11*vd + HK18;
float HK20 = HK11*ve + HK15*vd;
float HK21 = HK20 - HK7*vn;
float HK22 = q0*q3;
float HK23 = q1*q2;
float HK24 = 2*Tbs(0,1);
float HK25 = powf(q1, 2);
float HK26 = powf(q2, 2);
float HK27 = -HK26;
float HK28 = powf(q0, 2);
float HK29 = powf(q3, 2);
float HK30 = HK28 - HK29;
float HK31 = q0*q2;
float HK32 = q1*q3;
float HK33 = 2*Tbs(0,2);
float HK34 = HK33*(HK31 + HK32) + Tbs(0,0)*(HK25 + HK27 + HK30);
float HK35 = -HK24*(HK22 - HK23) + HK34;
float HK36 = q0*q1;
float HK37 = q2*q3;
float HK38 = -HK25;
float HK39 = 2*Tbs(0,0);
float HK40 = HK39*(HK22 + HK23) + Tbs(0,1)*(HK26 + HK30 + HK38);
float HK41 = -HK33*(HK36 - HK37) + HK40;
float HK42 = HK24*(HK36 + HK37) + Tbs(0,2)*(HK27 + HK28 + HK29 + HK38);
float HK43 = -HK39*(HK31 - HK32) + HK42;
float HK44 = 2*HK12;
float HK45 = 2*HK16 + 2*ve*(-HK0 - HK1 + HK2);
float HK46 = 2*HK18 + 2*vd*(HK10 - HK8 - HK9);
float HK47 = 2*HK20 + 2*vn*(-HK4 - HK5 + HK6);
float HK48 = HK24*(-HK22 + HK23) + HK34;
float HK49 = HK33*(-HK36 + HK37) + HK40;
float HK50 = HK39*(-HK31 + HK32) + HK42;
float HK51 = HK44*P(0,0) + HK45*P(0,1) + HK46*P(0,2) + HK47*P(0,3) + HK48*P(0,4) + HK49*P(0,5) + HK50*P(0,6);
float HK52 = powf(range, -2);
float HK53 = HK44*P(0,6) + HK45*P(1,6) + HK46*P(2,6) + HK47*P(3,6) + HK48*P(4,6) + HK49*P(5,6) + HK50*P(6,6);
float HK54 = HK44*P(0,5) + HK45*P(1,5) + HK46*P(2,5) + HK47*P(3,5) + HK48*P(4,5) + HK49*P(5,5) + HK50*P(5,6);
float HK55 = HK44*P(0,4) + HK45*P(1,4) + HK46*P(2,4) + HK47*P(3,4) + HK48*P(4,4) + HK49*P(4,5) + HK50*P(4,6);
float HK56 = HK44*P(0,2) + HK45*P(1,2) + HK46*P(2,2) + HK47*P(2,3) + HK48*P(2,4) + HK49*P(2,5) + HK50*P(2,6);
float HK57 = 2*HK52;
float HK58 = HK44*P(0,3) + HK45*P(1,3) + HK46*P(2,3) + HK47*P(3,3) + HK48*P(3,4) + HK49*P(3,5) + HK50*P(3,6);
float HK59 = HK44*P(0,1) + HK45*P(1,1) + HK46*P(1,2) + HK47*P(1,3) + HK48*P(1,4) + HK49*P(1,5) + HK50*P(1,6);
float HK60 = HK13/(HK17*HK57*HK59 + HK19*HK56*HK57 + HK21*HK57*HK58 + HK35*HK52*HK55 + HK41*HK52*HK54 + HK43*HK52*HK53 + HK44*HK51*HK52 + R_LOS);
