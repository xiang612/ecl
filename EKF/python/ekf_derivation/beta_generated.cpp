// Sub Expressions
float HK0 = vn - vwn;
float HK1 = ve - vwe;
float HK2 = HK0*q0 + HK1*q3 - q2*vd;
float HK3 = q0*q2 - q1*q3;
float HK4 = 2*vd;
float HK5 = q0*q3;
float HK6 = q1*q2;
float HK7 = 2*HK5 + 2*HK6;
float HK8 = powf(q0, 2);
float HK9 = powf(q3, 2);
float HK10 = HK8 - HK9;
float HK11 = powf(q1, 2);
float HK12 = powf(q2, 2);
float HK13 = HK11 - HK12;
float HK14 = HK10 + HK13;
float HK15 = HK0*HK14 + HK1*HK7 - HK3*HK4;
float HK16 = 1.0F/HK15;
float HK17 = q0*q1 + q2*q3;
float HK18 = HK10 - HK11 + HK12;
float HK19 = HK16*(-2*HK0*(HK5 - HK6) + HK1*HK18 + HK17*HK4);
float HK20 = -HK0*q3 + HK1*q0 + q1*vd;
float HK21 = -HK19*HK2 + HK20;
float HK22 = 2*HK16;
float HK23 = HK0*q1 + HK1*q2 + q3*vd;
float HK24 = HK0*q2 - HK1*q1 + q0*vd;
float HK25 = -HK19*HK23 + HK24;
float HK26 = HK19*HK24 + HK23;
float HK27 = HK19*HK20 + HK2;
float HK28 = HK14*HK19 + 2*HK5 - 2*HK6;
float HK29 = HK16*HK28;
float HK30 = HK19*HK7;
float HK31 = HK17 + HK19*HK3;
float HK32 = HK13 + HK30 - HK8 + HK9;
float HK33 = 2*HK31;
float HK34 = 2*HK26;
float HK35 = 2*HK25;
float HK36 = 2*HK27;
float HK37 = 2*HK21;
float HK38 = HK28*P(0,22) - HK28*P(0,4) + HK32*P(0,23) - HK32*P(0,5) + HK33*P(0,6) + HK34*P(0,2) + HK35*P(0,1) - HK36*P(0,3) + HK37*P(0,0);
float HK39 = powf(HK15, -2);
float HK40 = -HK28*P(4,6) + HK28*P(6,22) - HK32*P(5,6) + HK32*P(6,23) + HK33*P(6,6) + HK34*P(2,6) + HK35*P(1,6) - HK36*P(3,6) + HK37*P(0,6);
float HK41 = HK32*P(5,23);
float HK42 = HK28*P(22,23) - HK28*P(4,23) + HK32*P(23,23) + HK33*P(6,23) + HK34*P(2,23) + HK35*P(1,23) - HK36*P(3,23) + HK37*P(0,23) - HK41;
float HK43 = HK32*HK39;
float HK44 = HK28*P(4,22);
float HK45 = HK28*P(22,22) + HK32*P(22,23) - HK32*P(5,22) + HK33*P(6,22) + HK34*P(2,22) + HK35*P(1,22) - HK36*P(3,22) + HK37*P(0,22) - HK44;
float HK46 = HK28*HK39;
float HK47 = -HK28*P(4,5) + HK28*P(5,22) - HK32*P(5,5) + HK33*P(5,6) + HK34*P(2,5) + HK35*P(1,5) - HK36*P(3,5) + HK37*P(0,5) + HK41;
float HK48 = -HK28*P(4,4) + HK32*P(4,23) - HK32*P(4,5) + HK33*P(4,6) + HK34*P(2,4) + HK35*P(1,4) - HK36*P(3,4) + HK37*P(0,4) + HK44;
float HK49 = HK28*P(2,22) - HK28*P(2,4) + HK32*P(2,23) - HK32*P(2,5) + HK33*P(2,6) + HK34*P(2,2) + HK35*P(1,2) - HK36*P(2,3) + HK37*P(0,2);
float HK50 = HK28*P(1,22) - HK28*P(1,4) + HK32*P(1,23) - HK32*P(1,5) + HK33*P(1,6) + HK34*P(1,2) + HK35*P(1,1) - HK36*P(1,3) + HK37*P(0,1);
float HK51 = HK28*P(3,22) - HK28*P(3,4) + HK32*P(3,23) - HK32*P(3,5) + HK33*P(3,6) + HK34*P(2,3) + HK35*P(1,3) - HK36*P(3,3) + HK37*P(0,3);
float HK52 = HK16/(HK33*HK39*HK40 + HK34*HK39*HK49 + HK35*HK39*HK50 - HK36*HK39*HK51 + HK37*HK38*HK39 + HK42*HK43 - HK43*HK47 + HK45*HK46 - HK46*HK48 + R_BETA);


// Observation Jacobians
H_BETA(0) = HK21*HK22;
H_BETA(1) = HK22*HK25;
H_BETA(2) = HK22*HK26;
H_BETA(3) = -HK22*HK27;
H_BETA(4) = -HK29;
H_BETA(5) = HK16*(HK18 - HK30);
H_BETA(6) = HK22*HK31;
H_BETA(7) = 0;
H_BETA(8) = 0;
H_BETA(9) = 0;
H_BETA(10) = 0;
H_BETA(11) = 0;
H_BETA(12) = 0;
H_BETA(13) = 0;
H_BETA(14) = 0;
H_BETA(15) = 0;
H_BETA(16) = 0;
H_BETA(17) = 0;
H_BETA(18) = 0;
H_BETA(19) = 0;
H_BETA(20) = 0;
H_BETA(21) = 0;
H_BETA(22) = HK29;
H_BETA(23) = HK16*HK32;


// Kalman gains
Kfusion(0) = HK38*HK52;
Kfusion(1) = HK50*HK52;
Kfusion(2) = HK49*HK52;
Kfusion(3) = HK51*HK52;
Kfusion(4) = HK48*HK52;
Kfusion(5) = HK47*HK52;
Kfusion(6) = HK40*HK52;
Kfusion(7) = HK52*(-HK28*P(4,7) + HK28*P(7,22) - HK32*P(5,7) + HK32*P(7,23) + HK33*P(6,7) + HK34*P(2,7) + HK35*P(1,7) - HK36*P(3,7) + HK37*P(0,7));
Kfusion(8) = HK52*(-HK28*P(4,8) + HK28*P(8,22) - HK32*P(5,8) + HK32*P(8,23) + HK33*P(6,8) + HK34*P(2,8) + HK35*P(1,8) - HK36*P(3,8) + HK37*P(0,8));
Kfusion(9) = HK52*(-HK28*P(4,9) + HK28*P(9,22) - HK32*P(5,9) + HK32*P(9,23) + HK33*P(6,9) + HK34*P(2,9) + HK35*P(1,9) - HK36*P(3,9) + HK37*P(0,9));
Kfusion(10) = HK52*(HK28*P(10,22) - HK28*P(4,10) + HK32*P(10,23) - HK32*P(5,10) + HK33*P(6,10) + HK34*P(2,10) + HK35*P(1,10) - HK36*P(3,10) + HK37*P(0,10));
Kfusion(11) = HK52*(HK28*P(11,22) - HK28*P(4,11) + HK32*P(11,23) - HK32*P(5,11) + HK33*P(6,11) + HK34*P(2,11) + HK35*P(1,11) - HK36*P(3,11) + HK37*P(0,11));
Kfusion(12) = HK52*(HK28*P(12,22) - HK28*P(4,12) + HK32*P(12,23) - HK32*P(5,12) + HK33*P(6,12) + HK34*P(2,12) + HK35*P(1,12) - HK36*P(3,12) + HK37*P(0,12));
Kfusion(13) = HK52*(HK28*P(13,22) - HK28*P(4,13) + HK32*P(13,23) - HK32*P(5,13) + HK33*P(6,13) + HK34*P(2,13) + HK35*P(1,13) - HK36*P(3,13) + HK37*P(0,13));
Kfusion(14) = HK52*(HK28*P(14,22) - HK28*P(4,14) + HK32*P(14,23) - HK32*P(5,14) + HK33*P(6,14) + HK34*P(2,14) + HK35*P(1,14) - HK36*P(3,14) + HK37*P(0,14));
Kfusion(15) = HK52*(HK28*P(15,22) - HK28*P(4,15) + HK32*P(15,23) - HK32*P(5,15) + HK33*P(6,15) + HK34*P(2,15) + HK35*P(1,15) - HK36*P(3,15) + HK37*P(0,15));
Kfusion(16) = HK52*(HK28*P(16,22) - HK28*P(4,16) + HK32*P(16,23) - HK32*P(5,16) + HK33*P(6,16) + HK34*P(2,16) + HK35*P(1,16) - HK36*P(3,16) + HK37*P(0,16));
Kfusion(17) = HK52*(HK28*P(17,22) - HK28*P(4,17) + HK32*P(17,23) - HK32*P(5,17) + HK33*P(6,17) + HK34*P(2,17) + HK35*P(1,17) - HK36*P(3,17) + HK37*P(0,17));
Kfusion(18) = HK52*(HK28*P(18,22) - HK28*P(4,18) + HK32*P(18,23) - HK32*P(5,18) + HK33*P(6,18) + HK34*P(2,18) + HK35*P(1,18) - HK36*P(3,18) + HK37*P(0,18));
Kfusion(19) = HK52*(HK28*P(19,22) - HK28*P(4,19) + HK32*P(19,23) - HK32*P(5,19) + HK33*P(6,19) + HK34*P(2,19) + HK35*P(1,19) - HK36*P(3,19) + HK37*P(0,19));
Kfusion(20) = HK52*(HK28*P(20,22) - HK28*P(4,20) + HK32*P(20,23) - HK32*P(5,20) + HK33*P(6,20) + HK34*P(2,20) + HK35*P(1,20) - HK36*P(3,20) + HK37*P(0,20));
Kfusion(21) = HK52*(HK28*P(21,22) - HK28*P(4,21) + HK32*P(21,23) - HK32*P(5,21) + HK33*P(6,21) + HK34*P(2,21) + HK35*P(1,21) - HK36*P(3,21) + HK37*P(0,21));
Kfusion(22) = HK45*HK52;
Kfusion(23) = HK42*HK52;


